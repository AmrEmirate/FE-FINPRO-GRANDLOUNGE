...
interface PropertyData { id: string; name: string; description: string; categoryId: string; cityId: string; zipCode: string; mainImage: string | null; amenities: Amenity[]; images: PropertyImage[]; latitude?: number; longitude?: number }
...
const propertyFormSchema = z.object({
  ...
  zipCode: z.string().min(5).max(5, { message: "Zip code must be 5 characters." }),
...
export default function EditPropertyPage({ params }: { params: { id: string } }) {
  ...
  async function onSubmit(values: z.infer<typeof propertyFormSchema>) {
    ...
    try {
      await apiHelper.patch(`/properties/my-properties/${params.id}`, formData, { headers: { 'Content-Type': 'multipart/form-data' } })
      toast({ title: "Success!", description: "Property has been updated successfully." })
      router.push("/tenant/properties")
      router.refresh()
    } catch (error: any) {
      toast({ variant: "destructive", title: "Update Failed", description: error.response?.data?.message || "Something went wrong." })
    } finally {
      setIsLoading(false)
    }
  }
  ...
  if (isFetching) return <div className="flex justify-center items-center h-screen"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>
        ...
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              <FormField control={form.control} name="name" render={({ field }) => (<FormItem><FormLabel>Property Name</FormLabel><FormControl><Input placeholder="e.g. Grand Hyatt Villa" {...field} /></FormControl><FormMessage /></FormItem>)} />
              <FormField control={form.control} name="description" render={({ field }) => (<FormItem><FormLabel>Description</FormLabel><FormControl><Textarea rows={5} placeholder="Describe your property..." {...field} /></FormControl><FormMessage /></FormItem>)} />
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <FormField control={form.control} name="cityId" render={({ field }) => (<FormItem><FormLabel>City</FormLabel><Select onValueChange={handleCityChange} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Select a city" /></SelectTrigger></FormControl><SelectContent>{cities.map(city => (<SelectItem key={city.id} value={city.id}>{city.name}</SelectItem>))}</SelectContent></Select><FormMessage /></FormItem>)} />
              </div>
              ...
              <FormItem><FormLabel>Location</FormLabel><LocationPicker onLocationSelect={({ lat, lng }) => { form.setValue("latitude", lat); form.setValue("longitude", lng) }} initialPosition={form.getValues("latitude") && form.getValues("longitude") ? { lat: form.getValues("latitude")!, lng: form.getValues("longitude")! } : undefined} cityCoordinates={selectedCity ? { lat: selectedCity.latitude, lng: selectedCity.longitude } : undefined} /></FormItem>
              ...
              <div className="flex justify-end gap-4 pt-4"><Button type="button" variant="outline" onClick={() => router.back()}>Cancel</Button><Button type="submit" disabled={isLoading}>{isLoading && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}{isLoading ? "Saving..." : "Save Changes"}</Button></div>
            </form>
          </Form>
        </CardContent>
      </Card>
